...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        self.one_neck = one_neck
        self.selected_layers = [8, 9, 10, 11]  # 选择最后四层

        if self.ms_features and (self.one_neck == False):
            
            self.neck_list = nn.ModuleList([
                self.get_one_neck(embed_dim, out_chans) for _ in range(len(self.selected_layers))
            ])
...
if __name__ == "__main__":
    ...
    input_dummy = torch.randn((2, 3, 1024, 1024))

    output = image_encoder(input_dummy)